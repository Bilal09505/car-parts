...
import { VehicleModel } from '../../core/models';
    ...
    <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" (click)="close.emit()">
      <div class="bg-white rounded shadow-lg w-full max-w-md p-4" (click)="$event.stopPropagation()">
        <div class="flex items-center justify-between mb-3">
          <h2 class="font-bold text-slate-800">Manage Vehicles</h2>
        ...
        <div class="max-h-64 overflow-y-auto border border-gray-200 rounded mb-3 divide-y divide-gray-100">
          @for (t of vehicles(); track t.id) {
            ...
            <div class="px-3 py-3 text-sm text-gray-400">No vehicles yet — add one below.</div>
          }
        </div>
...
export class VehicleModalComponent {
  private typeService = inject(VehicleService);
  close = output<void>();
  ...
  constructor() {
    this.typeService.list().subscribe((list) => this.vehicles.set(list));
  }
  ...
  startEdit(t: VehicleModel) {
    this.editingId.set(t.id!);
    this.editingName = t.name;
  }

  async saveEdit(t: VehicleModel) {
